...
function BootShareContent({ item, currentUser, deleteBootShareMutation }) {
  return (
...
function ReviewContent({ item, venue, currentUser, followedUserRatings, deleteReviewMutation, deletePhotoMutation }) {
  return (
...
function FavoriteContent({ item, venue, currentUser }) {
  return (
    <Card className={`hover:shadow-lg transition-shadow ${item.isBoosted ? 'border-amber-300 border-2' : item.isPopular ? 'border-red-300 border-2' : ''}`}>
      <CardContent className="p-6">
        <div className="flex items-start justify-between mb-4">
